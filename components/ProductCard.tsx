...
export default function ProductCard({
  ...
  const imageClass =
    'relative block aspect-[3/4] overflow-hidden rounded-xl bg-gray-100 mb-4 shadow-sm group-hover:shadow-xl transition-all duration-300';
  ...
  return (
    <div className="group bg-transparent rounded-lg h-full flex flex-col hover-lift">
      {porials ? (
        <div className={`${imageClass} cursor-default`}>{imageBody}</div>
      ) : (
        <Link href={`/product/${slug}`} className={imageClass}>
          {imageBody}
        </Link>
      )}
      ...
      <div className="flex flex-col flex-grow">
        {porials ? (
          <h3 className="font-serif text-lg leading-tight text-gray-900 mb-1 line-clamp-2">{name}</h3>
        ) : (
        ...
        {!porials && colorVariants.length > 0 && (
